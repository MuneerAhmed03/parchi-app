...
const Parchi: FC<ParchiProps> = ({ playerId, selectedPlayer, setSelectedPlayer }) => {
  ...
  return (
    <div className={`relative w-fit group hover:cursor-pointer transition-transform duration-500 hover:rotate-6 hover:-translate-y-2 hover:scale-110 ease-in-out hover:cursor h-48 mr-2 ${isSelected && "border-2 border-black"}`} onClick={() => setSelectedPlayer(playerId)}>
      {/* Title on top */}
      ...
      {isSelected && (
        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 transition-all duration-500 ease-in-out">
          <button
            className="bg-green-500 text-white border border-black font-bold py-1 px-3 text-xl rounded shadow-lg tracking-wide animate-bounce"
            onClick={(e) => {
